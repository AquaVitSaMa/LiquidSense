...
public class InvClean extends Module {
    ...
    private int getBlockStacksAmount() {
        ...
        for (int i = 9; i < 45; i++) {
            ...
            if (itemStack != null && itemStack.getItem() instanceof ItemBlock && !itemStack.getUnlocalizedName().contains("flower")) amount += 1;
        }
        return amount;
    }
    ...
    private int getEnchantedBooksAmount() {
        ...
        for (int i = 9; i < 45; i++) {
            ItemStack itemStack = mc.thePlayer.inventoryContainer.getSlot(i).getStack();
            if (itemStack != null && itemStack.getUnlocalizedName().equals("item.enchantedBook")) amount += 1;
        }
        return amount;
    }
    ...
    private int getWaterBucketsAmount() {
        ...
        for (int i = 9; i < 45; i++) {
            ItemStack itemStack = mc.thePlayer.inventoryContainer.getSlot(i).getStack();
            if (itemStack != null && itemStack.getUnlocalizedName().equals("item.bucketWater")) amount += 1;
        }
        return amount;
    }
    ...
    private int getLavaBucketsAmount() {
        ...
        for (int i = 9; i < 45; i++) {
            ItemStack itemStack = mc.thePlayer.inventoryContainer.getSlot(i).getStack();
            if (itemStack != null && itemStack.getUnlocalizedName().equals("item.bucketLava")) amount += 1;
        }
        return amount;
    }
    ...
    private Map<Integer, ItemStack> items(final int start, final int end) {
        ...
        for (int i = end - 1; i >= start; --i) {
            ...
            if (System.currentTimeMillis() - ((IItemStack)(Object)itemStack).getItemDelay() >= itemDelayValue.get()) items.put(i, itemStack);
        }
        return items;
    }
                        ...
                        if (bestWeapon == -1) {
                            ...
                            for (final AttributeModifier attributeModifier : itemStack.getAttributeModifiers().get("generic.attackDamage")) {
                                ...
                                for (final AttributeModifier anotherAttributeModifier : bestStack.getAttributeModifiers().get("generic.attackDamage")) {
                                    if (damage <= anotherAttributeModifier.getAmount() + 1.25 *
                                            ItemUtils.getEnchantment(bestStack, Enchantment.sharpness)) bestWeapon = index;
                                }
                            }
                        }
                    }
                ...
                for (ItemStack stack : mc.thePlayer.inventory.mainInventory) {
                    if (stack != null) {
                        ...
                        if (item instanceof ItemBucket && ((ItemBucket) item).isFull == Blocks.flowing_water &&
                            ...
                            boolean replaceCurr = slotStack == null || !(slotStack.getItem() instanceof ItemBucket) ||
    ...
    private void sortHotbar() {
        for (int index = 0; index < 9; index++) {
            ...
            if (bestItem != null && bestItem != index) {
                boolean openInventory = !(mc.currentScreen instanceof GuiInventory) && simulateInventory.get();
